...
function ProductPage({products,addToCart,match,history}){
  const product = products.find((item) => item.id === parseFloat(match.params.id));
  return(
    <div className="container">
      <div className="product-box">
        <img className="product-img" src={product.url} alt={`${product.manufacture} ${product.model}`}/>
        <div className="product-info">
          <div style={{display:'flex', justifyContent: 'space-between'}}>
            <h3 className="product-title">{product.manufacture}</h3>
            {product.comments ? <StarBox rating={product.comments.reduce((acc,item)=>{
              return acc + parseInt(item.rating) / product.comments.length},0)}/> : <StarBox/>}
          </div>
          ...
          <button className="btn inline-block" 
                  onClick={(event)=>{
                    addToCart(product);
                    let interval = setInterval(()=> event.target.nextSibling.className = 'alert-success inline-block', 500);
                    setTimeout(()=>{
          ...
          <button className="btn rate inline-block" 
                  onClick={()=>{
